...
.wrapper {
	...
	background: #0e0e10;
	filter: url('~/assets/svg/optimized/loader-filter.svg#filter');
}
...
.ball {
	...
	&:nth-child(1) {
		transform: translate(-$radius * 1.1, -50%);
...
@keyframes x-axis {
	0% {
		transform: translate(-$radius * 1.6, -50%);
	}
	100% {
		transform: translate($radius * 1.6, -50%);
	}
...
@keyframes x-axis-lateral {
	0% {
		transform: translate(-$radius * 0.6, -50%);
	}
	100% {
		transform: translate($radius * 0.6, -50%);
	}
